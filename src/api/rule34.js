...
class Rule34API extends BaseImageAPI {
  constructor(config) {
    super(config);
    this.name = "rule34";
  }
  ...
  async search({
    ...
    try {
      ...
      if (!res.ok) {
        console.error(`[Rule34] HTTP ${res.status}: ${res.statusText}`);
        return [];
      }
      ...
      console.error("[Rule34] Search error:", err.message);
      return [];
    }
  }
  ...
  normalize(raw) {
    ...
    // Ignorar posts inactivos (si la API los devuelve)[cite: 5]
